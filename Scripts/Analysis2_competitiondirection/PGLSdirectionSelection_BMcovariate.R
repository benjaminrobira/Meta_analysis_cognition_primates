...
summaryData$Neocortex.log  <- log(summaryData$Neocortex)

summaryData$Hippocampus.log <- log(summaryData$Hippocampus)

summaryData$Cerebellum.log <- log(summaryData$Cerebellum )

summaryData$Striatum.log <- log(summaryData$Striatum/summaryData$Bodymass)

summaryData$MOB.log <- log(summaryData$MOB)
...
traitToStudy=c("EQ", "Brain.log", "Hippocampus.log", "Neocortex.log", "Cerebellum.log", "Striatum.log", "MOB.log")
traitName=c("EQ", "Brain (log)", "Hippocampus (log)", "Neocortex (log)", "Cerebellum (log)", "Striatum (log)", "MOB (log)")
vifVector <- rep(NA, times=length(traitName))  

results.df <- as.data.frame(matrix(NA, nrow=6*length(traitToStudy), ncol=7))
...
for(a in 1:length(traitToStudy)){
  ...
  if(traitToStudy[a] == "EQ"){
    ...
    results.df[(firstRowWrite+1):(firstRowWrite+5), 1] <- c("Intercept", "% of overlapped home range", "Number of sympatric frugivorous (sqrt)", "Bodymass (log)", "Lambda") 
  }
  ...
  modelBrain <- phylolm(formula = Trait ~ Overlap_average + Number_species_cooccurrence.sqrt + Bodymass.log, 
                        data=dataRangePrimate_rdc ,phy=phyloConsensus,model="lambda", measurement_error=FALSE,boot=repetitionBootstrap)
  
  #Complete df results
  
  #commented is when I used pgls + phylolm
  results.df[(firstRowWrite+1):(firstRowWrite+4), 2] <- roundIntelligent(summary(modelBrain)$coefficients[,1], digit=2) #roundIntelligent(summary(modelBrain)$tTable[,1], digit=2)
  results.df[(firstRowWrite+1):(firstRowWrite+4), 3] <- roundIntelligent(summary(modelBrain)$coefficients[,4], digit=2) #roundIntelligent(CI$coef[,1], digit=2)
  results.df[(firstRowWrite+1):(firstRowWrite+4), 4] <- roundIntelligent(summary(modelBrain)$coefficients[,5], digit=2) #roundIntelligent(CI$coef[,3], digit=2)
  results.df[(firstRowWrite+1):(firstRowWrite+4), 5] <- roundIntelligent(summary(modelBrain)$coefficients[,2], digit=2) #roundIntelligent(summary(modelBrain)$tTable[,2], digit=2)
  results.df[(firstRowWrite+1):(firstRowWrite+4), 6] <- c("-", roundIntelligent(summary(modelBrain)$coefficients[2:4,3], digit=2))
  results.df[(firstRowWrite+1):(firstRowWrite+4), 7] <- c("-", roundIntelligent(summary(modelBrain)$coefficients[2:4,6], digit=2))
  ...
  # lambda <- paste(round(summary(modelBrainPgls)$param.CI$lambda$opt, digit=2), " [",val1,",",val2,"]", sep="")
  #         
  results.df[firstRowWrite+5,2] <- roundIntelligent(summary(modelBrain)$optpar)#bootmean[5])
  results.df[firstRowWrite+5,3] <- roundIntelligent(summary(modelBrain)$bootconfint95[1,6])#CI$corStruct[1,1])
  results.df[firstRowWrite+5,4] <- roundIntelligent(summary(modelBrain)$bootconfint95[2,6])#CI$corStruct[1,3])
...
test1 <- get(paste("modelBrain",  "EQ", sep="_"))
test2 <- get(paste("modelBrain", "Brain", sep="_"))
...
for(a in 1:length(traitToStudy)){
  ...
  for(r in 1:nrow(dataRangePrimate_rdc)){
    ...
    if(traitToStudy[a] == "EQ"){
      ...
      modelBrain <- phylolm(Trait ~ Overlap_average + Number_species_cooccurrence.sqrt + Bodymass.log, phy=phyloConsensus, data = dataRangePrimate_rdc2, model = "lambda", measurement_error=FALSE)
    }
...
pdf("Plots/selectionGradientPGLS_BMcovariate.pdf", height=40, width=18)
source("T:/Saved_PhD/Empirical_analysis/Scripts&Functions/Functions/toolbox.R")
...
for(i in 1:length(traitToStudy)){ 
  ...
  if(i != 2){#Brain do nothing
    ...
    if(traitToStudy[i] == "EQ"){
      modelBrain <- phylolm(formula = Trait ~ Overlap_average.z + Number_species_cooccurrence.sqrt, 
                            data=dataRangePrimate_rdc, phy=phyloConsensus, model="lambda", measurement_error=FALSE, boot=repetitionBootstrap)
    }else{
      modelBrain <- phylolm(formula = Trait ~ Overlap_average.z + Number_species_cooccurrence.sqrt + Bodymass.log, 
                            data=dataRangePrimate_rdc, phy=phyloConsensus, model="lambda", measurement_error=FALSE, boot=repetitionBootstrap)
    }
    ...
    if(traitToStudy[i] == "EQ"){
      ...
      modelBrain <- phylolm(formula = Trait ~ Overlap_average + Number_species_cooccurrence.sqrt.z + Bodymass.log, 
                            data=dataRangePrimate_rdc ,phy=phyloConsensus,model="lambda",measurement_error=FALSE,boot=repetitionBootstrap)
    }
...
save.image("REnvironments/PGLSdirectionSelection_BMcovariate.RData")